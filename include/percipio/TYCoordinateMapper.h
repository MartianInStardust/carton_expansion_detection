    ...
    uint32_t mappedW, uint32_t mappedH, uint16_t *mappedDepth)
{
   ...
   TYMAP_CHECKRET(TYMapPoint3dToDepthImage(
                      color_calib, p3d, depthW * depthH, mappedW, mappedH, mappedDepth),
                  p3d);
   free(p3d);
   return TY_STATUS_OK;
}
...
static inline TY_STATUS MapColorToDepthLookupTable(const TY_CAMERA_CALIB_INFO *depth_calib,
                                                   uint32_t depthW, uint32_t depthH, const uint16_t *depth,
                                                   const TY_CAMERA_CALIB_INFO *color_calib,
                                                   uint32_t rgbW, uint32_t rgbH, const uint8_t *inRgb,
                                                   uint8_t *mappedRgb, std::vector<uint32_t> &allInputRgbIndex, std::map<int, std::vector<uint32_t>> &allOutPutPoints)
{
   TY_PIXEL_DESC *lut = (TY_PIXEL_DESC *)malloc(sizeof(TY_PIXEL_DESC) * depthW * depthH);
   TYMAP_CHECKRET(TYCreateDepthToColorCoordinateLookupTable(
                      depth_calib, depthW, depthH, depth,
   ...
   // uint32_t *pixel = (uint32_t *)malloc(rgbW * rgbH);
   std::vector<uint32_t> pixel(rgbW * rgbH);
   for (uint32_t depthr = 0; depthr < depthH; depthr++)
      for (uint32_t depthc = 0; depthc < depthW; depthc++)
      {
         uint32_t search_depth_index = depthr * depthW + depthc;
         // uint32_t depth_index = depthW * depthr * 3 + depthc * 3;
         uint32_t depth_index = depthW * depthr + depthc;
         TY_PIXEL_DESC *plut = &lut[search_depth_index];
         uint32_t rgb_index = rgbW * plut->y + plut->x;
         uint8_t *outPtr = &mappedRgb[depthW * depthr * 3 + depthc * 3];
         if (plut->x < 0 || plut->x >= (int)rgbW || plut->y < 0 || plut->y >= (int)rgbH)
         {
            // pixel[rgb_index] = 0;
   ...
   uint32_t range_cloud_index = 0;
   uint32_t times = 0;

   // assert(allInputRgbIndex.size() == 4);  // Only limited to the four corners

   for (auto value : allInputRgbIndex)
   {
      uint32_t inputRgb_outputCloud_x = value % rgbW;
      uint32_t inputRgb_outputCloud_y = value / rgbW;

      std::vector<uint32_t> temp_outPoints;
      uint32_t search_range = 8; //search range from image pixel-xy
      for (uint32_t m = inputRgb_outputCloud_y - search_range; m < inputRgb_outputCloud_y + search_range; m++)
         for (uint32_t n = inputRgb_outputCloud_x - search_range; n < inputRgb_outputCloud_x + search_range; n++)
         {
            uint32_t index = n + m * rgbW;
            if (pixel[index])
            {
               // range_cloud_index += pixel[index];
               temp_outPoints.push_back(pixel[index]);
               times++;
            }
            std::cout << "search index" << index << "outDepth index：" << pixel[index] << std::endl;
         }

      allOutPutPoints.insert(std::make_pair(value, temp_outPoints));
      temp_outPoints.clear();
      std::cout << "want to search image index:" << value << std::endl;
      std::cout << "times:" << times << "range_cloud_index" << range_cloud_index << std::endl;
   }
   std::cout << "input size:" << allInputRgbIndex.size() << "output size:" << allOutPutPoints.size() << std::endl;
   assert(allOutPutPoints.size() == allInputRgbIndex.size());

   free(lut);
   return TY_STATUS_OK;
}

static inline TY_STATUS MapColorToDepthLookupTable2(const TY_CAMERA_CALIB_INFO *depth_calib,
                                                    uint32_t depthW, uint32_t depthH, const uint16_t *depth,
                                                    const TY_CAMERA_CALIB_INFO *color_calib,
                                                    uint32_t rgbW, uint32_t rgbH, const uint8_t *inRgb,
                                                    uint8_t *mappedRgb, std::vector<uint32_t> &allInputRgbIndex, std::vector<uint32_t> &allOutPutPoints)
{
   TY_PIXEL_DESC *lut = (TY_PIXEL_DESC *)malloc(sizeof(TY_PIXEL_DESC) * depthW * depthH);
   TYMAP_CHECKRET(TYCreateDepthToColorCoordinateLookupTable(
                      depth_calib, depthW, depthH, depth,
   ...
   {
      if (pixel[value])
         allOutPutPoints.push_back(pixel[value]);
   }
   std::cout << "input size:" << allInputRgbIndex.size() << "output size:" << allOutPutPoints.size() << std::endl;
