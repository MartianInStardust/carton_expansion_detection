...
static inline int parseFrame(const TY_FRAME_DATA &frame, cv::Mat *pDepth, cv::Mat *pLeftIR, cv::Mat *pRightIR, cv::Mat *pColor, TY_ISP_HANDLE color_isp_handle = NULL)
{
   for (int i = 0; i < frame.validCount; i++)
   {
      ...
      if (pColor && frame.image[i].componentID == TY_COMPONENT_RGB_CAM)
      {
         ...
         {
            cv::Mat jpeg(frame.image[i].height, frame.image[i].width, CV_8UC1, frame.image[i].buffer);
            // *pColor = cv::imdecode(jpeg, CV_LOAD_IMAGE_COLOR);
            *pColor = cv::imdecode(jpeg, cv::IMREAD_COLOR);
         }
...
static void writePC_XYZ2(const cv::Point3f *pnts, const cv::Vec3b *color, size_t n, FILE *fp)
{
   if (color)
   {
      for (size_t i = 0; i < n; i++)
...
static void writePointCloud2(const cv::Point3f *pnts, const cv::Vec3b *color, size_t n, const char *file, int format)
{
   FILE *fp = fopen(file, "w");
   if (!fp)
   {
